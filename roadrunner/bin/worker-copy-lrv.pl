...
while (!$time_to_die) {
	...
	my $sth = $dbh->prepare("SELECT * FROM $db_table WHERE card_name NOT IN (SELECT DISTINCT(card_name) FROM $db_table WHERE job_status NOT IN ('ready', 'complete')) AND job_type = 'copy_lrv' AND job_status = 'ready' ORDER BY job_id ASC LIMIT 1");
	$sth->execute();
