...
sub copy_lrv {
	...
	foreach my $lrv (sort keys %args){
		...
		system("/usr/bin/scp ec2-user\@$remote_hostname\:$src_path\/$lrv $storage_path\/$card_id\/240p\/$dst_file");
	}
}
...
sub transcode_1080p_360p {
	...
	foreach my $mp4 (sort keys %args){
		...
		system("/usr/bin/scp ec2-user\@$remote_hostname\:$src_path\/$mp4 $storage_path\/$card_id\/1080p\/$dst_file");	
